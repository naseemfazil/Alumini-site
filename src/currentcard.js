...
class Currentcard extends Component {
    ...
    componentDidMount() {
        axios.get('http://localhost:3001/getCurrent')
            .then(response => {
        ...
        return (
            item.map((val, index) => {
                ...
                return (
                    ...
                    <Card className="card">
                        <CardHeader ><span style={{fontWeight: 'bold'}}>{val.Name}</span></CardHeader>
                        <CardBody>
                            <CardTitle> Department:{val.Department} </CardTitle>
                            <CardText>Area of Intrest:<span style={{fontWeight: 'bold'}}>{val.Areaofintrest}</span></CardText>
                        ...
                        <Button color="primary" onClick={this.butclick(val._id)}>More details</Button>
                    </Card>
                    </div>)
            }))
