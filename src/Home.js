...
export const From = (props) => {
  const plecementBut = () => {
    props.props.history.push("/placement");
  };
  ...
  // }
  const cpcurrentBut = () => {
    props.props.history.push("/cpcurrent");
  };
  const cppassoutBut = () => {
    props.props.history.push("/cppassout");
  };
  const logoutbut = () => {
      ...
      <AppBar position="static">
        <Toolbar>
          ...
          <Typography variant="h5" className="title">
            {/* Alumini */}
            <span className="font-weight-bold content-center">Alumini</span>
            .site
          </Typography>
          {localStorage.getItem("@Access_token") && (
            <Button onClick={plecementBut} color="inherit">
              Placement Information
            </Button>
          )}
          {/* <Button onClick={registerBut} color="inherit">Register</Button>  */}
          <Button onClick={cpcurrentBut} color="inherit">
            Current student Profile
          </Button>
          <Button onClick={cppassoutBut} color="inherit">
            PassedOut Profile
          </Button>
          <Button onClick={Adminbut} color="inherit">
            Admin
          </Button>
...
class Home extends Component {
  constructor(props) {
    ...
    axios.post("http://localhost:3001/getDashboard").then((response) => {
